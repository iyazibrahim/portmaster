...
**Stripe primary + Admin gateway (2026-09-24):** Admin Ops → Payment gateway selects primary checkout: `stripe` (default), `hitpay`, or `mock`. Resolution: Admin setting + env keys; missing keys fall back to mock. Stripe Checkout Sessions (`POST` create + webhook `POST /api/webhooks/stripe` on `checkout.session.completed`). HitPay webhook unchanged. **Mock Pay successfully / Payment failed buttons always remain** as Demo / fallback on the angler pay step. Dokploy must set `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET` (and optional `HITPAY_*`) on the compose stack; vars must be listed under `app.environment` in `docker-compose.yml`. Not needed in GitHub Actions — runtime only. Redeploy/restart after changing env. Seed default `payment_gateway=stripe`.
