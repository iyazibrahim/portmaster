...
**Scan check-in + camera permission (2026-09-21):** `actionScanToken` no longer calls `revalidatePath` — any revalidation re-renders the current Scan route, remounts the camera (phone re-prompts), and could throw a minified React error on Confirm. Camera uses a soft-release hub (30 min) so tab switches / remounts reuse the live stream instead of calling `getUserMedia` again. Camera is **manual Open only** (paste check-in works without camera). Scan/preview return `{ ok, error }` so phones see real GPS/jetty messages. Cookie consent persists via cookie + `localStorage` (Secure on HTTPS) and sits above the tab bar. Geofence errors use boarding copy with distance for CI/CO.
