...
**HitPay sandbox payments (2026-09-24):** Pass purchase can redirect to HitPay when `HITPAY_API_KEY` is set. Sandbox MYR methods default `grabpay_direct,shopee_pay,atome`; production later uses `fpx,duitnow` via env. Pass activates only on HMAC-verified webhook `POST /api/webhooks/hitpay`. Without API key, mock Pay success/fail remains. See `.env.example`. Mock confirm no longer requires a prior in-memory intent (fixes “Payment intent not found” after reserve). **Dokploy:** set `HITPAY_*` on the compose stack env; they must be listed under `app.environment` in `docker-compose.yml` (compose does not pass undeclared vars into the container). Not needed in GitHub Actions — runtime only. After changing Dokploy env, redeploy/restart the app container.
